...
public class ClientMainForm extends JFrame implements ActionListener, Runnable,MouseListener {
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      // 찾기 =================================
         ...
         for(int i=tp.cp.fbf.model.getRowCount()-1;i>=0;i--)
         {
            tp.cp.fbf.model.removeRow(i);
         }
   ...
   public void mouseClicked(MouseEvent e) {
      // TODO Auto-generated method stub
      if(e.getSource()==tp.cp.fbf.table)
      {
         if(e.getClickCount()==2)
         {
            int row=tp.cp.fbf.table.getSelectedRow();
            String title=tp.cp.fbf.model.getValueAt(row, 1).toString();
            ArrayList<BookVO> list=BookData.mList;
            for(int i=0;i<list.size();i++)
            {
               BookVO vo=list.get(i);
               if(vo.getTitle().equals(title))
               {
                  //tp.cp.dm.listBtn.setVisible(true);
                  tp.cp.dm.removeAll();
                  /*tp.cp.dm.init(title);
                  tp.cp.card.show(tp.cp, "DETAILMAIN");*/
                  tp.cp.dm.validate();
                  break;
               }
            }
